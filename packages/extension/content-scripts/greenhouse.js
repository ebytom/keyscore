// KeyScore - Greenhouse Content Script

(function() {
  'use strict';

  if (window.aiJobCopilotGreenhouse) return;
  window.aiJobCopilotGreenhouse = true;
  ...
  function extractJobData() {
    try {
      ...
      const company = urlParts[1] || document.querySelector('.company-name')?.textContent?.trim() || '';

      // Location
      const locationEl = document.querySelector('.location, .body--metadata');
      ...
      // Job type
      const type = extractJobType(description + ' ' + location);
